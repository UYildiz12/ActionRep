...
# Ask the user for the participant number
participant_number = simpledialog.askinteger("Input", "Enter participant number",
                                             parent=root, minvalue=0, maxvalue=100)


pygame.init()
dir_path = r"C:\Users\UmurY\OneDrive\Masaüstü\Multiarr_58vids\58videos"
...
def play_video(video_path):
    ...
    delay = int(1000 / fps)
    #Window should appaer in the middle of the screen
    cv2.namedWindow('Video', cv2.WINDOW_NORMAL)
    cv2.moveWindow('Video', 1920//2-width//2-200, 1080//2-height//2 -250)
...
def is_inside_circle(small_circle_center, small_circle_radius, big_circle_center, big_circle_radius):
    dx = small_circle_center[0] - big_circle_center[0]
    dy = small_circle_center[1] - big_circle_center[1]
    distance = np.sqrt(dx**2 + dy**2)
    big_circle_radius +=15 
    return distance <= (big_circle_radius - small_circle_radius)
...
red_circle = pygame.image.load(r'C:\Users\UmurY\OneDrive\Masaüstü\Multiarr_58vids\img1.PNG')
...
for i, message in enumerate(messages):
    ...
    while show_screen:
        ...
        for j, text_surface in enumerate(text_surfaces):
            x = (1920 - text_surface.get_width()) // 2
            y = start_y + j * text_surface.get_height() -220

            screen.blit(text_surface, (x, y))
        if i == 2:
            image_rect = red_circle.get_rect()
            image_rect.center = (1920 // 2, (start_y + total_height + image_rect.height+350) // 2)
            screen.blit(red_circle, image_rect)
        if i == 3:
            cap = cv2.VideoCapture(r'C:\Users\UmurY\OneDrive\Masaüstü\Multiarr_58vids\demovids\drag.mp4')  
            fps = cap.get(cv2.CAP_PROP_FPS)
            delay = int(1000 / fps)
            while pressed==False:  
                cap.set(cv2.CAP_PROP_POS_FRAMES, 0)
        ...
        pressed=False
        if i == 4:
            cap = cv2.VideoCapture(r'C:\Users\UmurY\OneDrive\Masaüstü\Multiarr_58vids\demovids\Doubleclick.mp4')  
            fps = cap.get(cv2.CAP_PROP_FPS)
            delay = int(1000 / fps)
            while pressed==False:
                print("heyo")
        ...
        pressed=False            
        if i == 5:
            cap = cv2.VideoCapture(r'C:\Users\UmurY\OneDrive\Masaüstü\Multiarr_58vids\demovids\Same.mkv')  
            fps = cap.get(cv2.CAP_PROP_FPS)
            delay = int(1000 / fps)
            while pressed==False:
                cap.set(cv2.CAP_PROP_POS_FRAMES, 0)
        ...
        pressed=False            
        if i == 6:
            cap = cv2.VideoCapture(r'C:\Users\UmurY\OneDrive\Masaüstü\Multiarr_58vids\demovids\similar.mp4')  
            fps = cap.get(cv2.CAP_PROP_FPS)
            delay = int(1000 / fps)
            while pressed==False:
                print("heyo")
        ...
        pressed=False            
        if i == 7:
            cap = cv2.VideoCapture(r'C:\Users\UmurY\OneDrive\Masaüstü\Multiarr_58vids\demovids\Done.mp4')  
            fps = cap.get(cv2.CAP_PROP_FPS)
            delay = int(1000 / fps)
            while pressed==False:
                cap.set(cv2.CAP_PROP_POS_FRAMES, 0)
...
with open(r'C:\Users\UmurY\OneDrive\Masaüstü\Multiarr_58vids\batches58.txt', 'r') as f:  
...
try:
    for batch_index, batch_indexes in enumerate(batches):
        ...
        frame_names = []
        #show_set(batch_videos) #-> Remove to skip firest showing of videos 
        ...
        for i, frame in enumerate(my_frames):
            ...
            pygame.draw.circle(mask_surface, (255, 255, 255, 128), (frame_width // 2, frame_height // 2), min(frame_width, frame_height) // 2.05)
        ...
        while running==True:
            if pygame.display.get_init() is None:
                raise SystemExit("Window closed early")
            ...
            all_inside = all(is_inside_circle(rect.center, rect.width // 2, circle_center, circle_radius) for rect in rects) # Checks whether the positions of all the small red circles are inside the big white circle
            ...
            if dragging and is_inside_circle(rects[dragged_frame_index].center, rects[dragged_frame_index].width // 2, circle_center, circle_radius):
                for i in range(len(frames)):
                    if i != dragged_frame_index and is_inside_circle(rects[i].center, rects[i].width // 2, circle_center, circle_radius):
    ...
    with open('batch_number.txt', 'w') as f:
...
#save dataframe as an excel file with regards to participant number asked in the beginning to C:\Users\UmurY\OneDrive\Masaüstü\Multiarr_58vids\Participantdata
df.to_excel(r'C:\Users\UmurY\OneDrive\Masaüstü\Multiarr_58vids\Participantdata\participant_{}.xlsx'.format(participant_number))
