...
pygame.init()
dir_path = r"C:\Users\UmurY\OneDrive\Masaüstü\Action_Clusters_Python\Raw_Files\VideosF\VideosFixed"
...
#A greedy algorithm that ensures every video appears with every other video in at least one batch and tries to 
# minimize number of batches. A less greedy version can be written easily if the list is small. However, if
# the number of elements is high this algorithm will work as well as any.
def get_batches(my_list, batch_size):
    all_pairs = set(itertools.combinations(my_list, 2))
    while all_pairs:
        batch = set()
        ...
        for pair in remaining_pairs:
            if len(set(batch).union(set(pair))) <= batch_size:
                batch.update(pair)
...
scale_factor=2.5
messages = [
    "Welcome to our experiment. Press the spacebar button to continue from now on.",
    "We will ask you to watch and compare the actions depicted in the videos on each pair of actions is.",
    "Firstly, you will watch a video for each action in the block. After you watched all videos you will see small circles representing each action you have just seen like in the following example:",
    "You will be asked to drag and drop each circle that is representing one of the videos you have watched in the big white circle.",
    "You can rewatch each video by double clicking the circle it is represented by. You have to rewatch each video at least once to be able to complete the task.",
    "When placing the represented videos in the white circle actions can overlap. If you think actions are exteremely similar you should place the center of the circles very close to eachother. ",
    "Still similar actions do not have to overlap you should place the circles based on how similar you think the actions depicted are. Remember it is the distance from the center that counts.",
    "Please be careful and closely examine the place of the each action and their distance to other actions. We strongly advise you to view each action as many times as you need to be sure of your selection"
]
red_circle = pygame.image.load(r'C:\Users\UmurY\OneDrive\Masaüstü\Multarr_Final\img1.PNG')
red_circle= pygame.transform.scale(red_circle, (280, 280))
pressed=False
#All from here are introductory slides to the tasks untill the while loop down
for i, message in enumerate(messages):
    ...
    while show_screen:
        ...
        if i == 3:
            cap = cv2.VideoCapture(r'C:\Users\UmurY\OneDrive\Masaüstü\Multarr_Final\demovids\drag.mp4')  
            fps = cap.get(cv2.CAP_PROP_FPS)
            delay = int(1000 / fps)
            while pressed==False:  
                cap.set(cv2.CAP_PROP_POS_FRAMES, 0)
        ...
        pressed=False
        if i == 4:
            cap = cv2.VideoCapture(r'C:\Users\UmurY\OneDrive\Masaüstü\Multarr_Final\demovids\Doubleclick.mp4')  
            fps = cap.get(cv2.CAP_PROP_FPS)
            delay = int(1000 / fps)
            while pressed==False:
                print("heyo")
        ...
        pressed=False            
        if i == 5:
            cap = cv2.VideoCapture(r'C:\Users\UmurY\OneDrive\Masaüstü\Multarr_Final\demovids\Same.mkv')  
            fps = cap.get(cv2.CAP_PROP_FPS)
            delay = int(1000 / fps)
            while pressed==False:
                print("heyo")
        ...
        pressed=False            
        if i == 6:
            cap = cv2.VideoCapture(r'C:\Users\UmurY\OneDrive\Masaüstü\Multarr_Final\demovids\similar.mp4')  
            fps = cap.get(cv2.CAP_PROP_FPS)
            delay = int(1000 / fps)
            while pressed==False:
                print("heyo")
        ...
        pressed=False            
        if i == 7:
            cap = cv2.VideoCapture(r'C:\Users\UmurY\OneDrive\Masaüstü\Multarr_Final\demovids\Done.mp4')  
            fps = cap.get(cv2.CAP_PROP_FPS)
            delay = int(1000 / fps)
            while pressed==False:
                cap.set(cv2.CAP_PROP_POS_FRAMES, 0)
...
# Most important parts as explained before uses the round_robin function to get a list of values that is a list of
# video itself these videos are designated ad current batch.
for batch in get_batches(avi_files,8):
    frame_names = []
    show_set(batch)
    #Use previous function to get the first frames of the batch
    my_frames=get_first_frames(batch)
    screen.fill((0, 0, 0))
    #Drawing the circle
    circle_radius = 355
    circle_thickness = 4  
        ...
        for event in pygame.event.get():
            if event.type == pygame.MOUSEBUTTONDOWN and button_rect.collidepoint(event.pos):    
                if all_inside and all_clicked:
                    ...
                    for avi_file in batch[:len(centers)]:
                ...
                if last_click_time is not None and current_time - last_click_time <= double_click_time_limit:
                    for i in range(len(frames)):
                       if rects[i].collidepoint(event.pos):
                           ...
                           video_path = os.path.join(dir_path, batch[i])  
