...
cd('/Users/burcuaysenurgen/Studies/SPM_ActionClustersBIDS_Subj04_Sess1/SPM/Clustering_PPC_volume_FINAL/');
load('sorted_action_betas.mat');
[avg_ranked_value_mat_100splits] = split_ranking_procedure_bilkent_BIDS_1session(sorted_action_betas);
...
%% PLOT SI of significant negatively correlated voxels
SI_all_voxels_NEG = [];
x = [1:100];
SI_NEG = [];
...
%    y_output = p(1).*(x.^5) + p(2).*(x.^4) + p(3).*(x.^3) + p(4).*(x.^2) + p(5).*(x) + p(6); % Fifth order
    ...
    SI_all_voxels_NEG(j) = SI_NEG;
end

save SI_all_voxels_NEG SI_all_voxels_NEG

f5 = figure;
histogram(SI_all_voxels_NEG,  'FaceColor', [1 0 0])
