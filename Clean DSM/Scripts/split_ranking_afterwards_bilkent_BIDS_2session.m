...
cd('/Volumes/BAURGEN/ActionClusters_ParametricModulator/subject01/SPM/Clustering_PPC_volume_FINAL/');
load('sorted_action_betas.mat');
[avg_ranked_value_mat_100splits] = split_ranking_procedure_bilkent_BIDS_2session(sorted_action_betas);
...
load('roi_voxels_indices_upd.mat');
voxel_indices_NEG_ALL_for_ROI = roi_voxels_indices_upd(voxel_indices_NEG_ALL);
save voxel_indices_NEG_ALL_for_ROI voxel_indices_NEG_ALL_for_ROI
...
[SORTED_SI_all_voxels_NEG_ALL,SORTED_NEG_SI_INDICES] = sort(SI_all_voxels_NEG_ALL);

first_800_voxel_indices = SORTED_NEG_SI_INDICES(1:800);
voxel_nos_to_go_to_clustering_800 = voxel_indices_NEG_ALL(first_800_voxel_indices);
save voxel_nos_to_go_to_clustering_800 voxel_nos_to_go_to_clustering_800
max_SI_800 = SORTED_SI_all_voxels_NEG_ALL(800);
save max_SI_800 max_SI_800

first_1000_voxel_indices = SORTED_NEG_SI_INDICES(1:1000);
voxel_nos_to_go_to_clustering_1000 = voxel_indices_NEG_ALL(first_1000_voxel_indices);
save voxel_nos_to_go_to_clustering_1000 voxel_nos_to_go_to_clustering_1000
max_SI_1000 = SORTED_SI_all_voxels_NEG_ALL(1000);
save max_SI_1000 max_SI_1000


first_1200_voxel_indices = SORTED_NEG_SI_INDICES(1:1200);
voxel_nos_to_go_to_clustering_1200 = voxel_indices_NEG_ALL(first_1200_voxel_indices);
save voxel_nos_to_go_to_clustering_1200 voxel_nos_to_go_to_clustering_1200
max_SI_1200 = SORTED_SI_all_voxels_NEG_ALL(1200);
save max_SI_1200 max_SI_1200



first_1400_voxel_indices = SORTED_NEG_SI_INDICES(1:1400);
voxel_nos_to_go_to_clustering_1400 = voxel_indices_NEG_ALL(first_1400_voxel_indices);
save voxel_nos_to_go_to_clustering_1400 voxel_nos_to_go_to_clustering_1400
max_SI_1400 = SORTED_SI_all_voxels_NEG_ALL(1400);
save max_SI_1400 max_SI_1400
...
%% create new volume (.nii file) that consists of voxels with selected SI values
voxel_indices_selectedVoxels_SI_for_ROI = voxel_indices_NEG_ALL_for_ROI(SORTED_NEG_SI_INDICES);
save voxel_indices_selectedVoxels_SI_for_ROI voxel_indices_selectedVoxels_SI_for_ROI

cd('/Volumes/BAURGEN/ActionClusters_ParametricModulator/subject01/ROIs/');
A = spm_read_vols(spm_vol('Clustering_PPC_volume_FINAL.nii'));
B = A;
B(:,:,:) = 0;
B(voxel_indices_selectedVoxels_SI_for_ROI) = 1;

C = spm_vol('Clustering_PPC_volume_FINAL.nii');
C.name = 'PPC_ALLNEG_SI_all.nii';
C.fname = 'PPC_ALLNEG_SI_all.nii';
cd('/Volumes/BAURGEN/ActionClusters_ParametricModulator/subject01/SPM/Clustering_PPC_volume_FINAL/');
spm_write_vol(C,B);

%% create new volume (.nii file) that consists of voxels with selected SI values = First 1200 voxels based on SI
voxel_indices_selectedVoxels_SI_for_ROI_1400 = voxel_indices_selectedVoxels_SI_for_ROI(1:1400);

cd('/Volumes/BAURGEN/ActionClusters_ParametricModulator/subject01/ROIs/');
A = spm_read_vols(spm_vol('Clustering_PPC_volume_FINAL.nii'));
B = A;
B(:,:,:) = 0;
B(voxel_indices_selectedVoxels_SI_for_ROI_1400) = 1;

C = spm_vol('Clustering_PPC_volume_FINAL.nii');
C.name = 'PPC_ALLNEG_SI_first1400.nii';
C.fname = 'PPC_ALLNEG_SI_first1400.nii';
cd('/Volumes/BAURGEN/ActionClusters_ParametricModulator/subject01/SPM/Clustering_PPC_volume_FINAL/');
spm_write_vol(C,B);
