...
function [] = create_regressor_array_for_newDesign_luminance(runNo)
...
for i = 1:length(regres_lum)
    regres_lum_all(full_count) = regres_lum(i);
    regres_lum_all(full_count+1) = regres_lum(i);
    regres_lum_all(full_count+2) = regres_lum(i);
    full_count = full_count + 3;
end

countReg = 2;
remain = 0;
regressor_array = [regres_lum(1) regres_lum(1) regres_lum(1)]; % the first three elements
totalsum = 0;
trialDur = 0;
...
for i = 1:55
    ...
    regressor_array = [regressor_array 0];
        ...
        for k = 1:no_of_reg
            if new_dur_array(i+1) ~= 0
                regressor_array = [regressor_array regres_lum(countReg)];
            else
                regressor_array = [regressor_array 0];
            end
        end  
...
regressor_array_lum = regressor_array;
save regressor_array_lum regressor_array_lum;
